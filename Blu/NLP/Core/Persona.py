...
def dictToPersona(data: dict) -> dict:
    # iterate through each key and value in data
    for key, value in data.items():
        # CASE: valuse is of type dict
        if isinstance(value,
                      dict):
            # CASE: key is "convo"
            if key == "convo":
                data[key] = dictToConvo(value)
            else:
...
class Persona(InformationFragment):
    ...
    def loadFromFile(self,
        ...
        with open(filePath,
            ...
            # take the file in and parse the json ot a dict
            data: dict[str, any] = json.load(file)
            # set the object dictionary to the parsed dictionary,
            # this makes the object extactly what was loaded
            self.__dict__ = dictToPersona(data)
